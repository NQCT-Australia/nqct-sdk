...
The installation of NQCT-SDK already includes `sphinx`. Thus, enter the associated virtual environment (changing the name to the venv name) and run the following commands in the root directory of the cloned repository:

```bash
conda activate nqct_env
...
```rst
Here is my link to a function :func:`nqct.utils.QuantumSession.QuantumSession.list_backends`
```
...
```rst
Here is my link to a function :func:`custom text <nqct.utils.QuantumSession.QuantumSession.list_backends>`
```
